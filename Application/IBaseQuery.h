...
{
	virtual std::shared_ptr<T> GetById(const std::string& id, std::vector<std::string> includes = {}) = 0;
	...
	virtual std::shared_ptr<T> GetFromCommand(SACommand& cmd, std::vector<std::string> includes = {}) = 0;

...
typedef int (*cubesql_connect_ssl_t)(csqldb **db, const char *host, int port, const char *username, const char *password, int timeout, const char *ssl_certificate_path);
...
typedef void (*cubesql_trace_t)(csqldb *db, cubesql_trace_callback trace, void *arg);
...
typedef int (*cubesql_send_data_t)(csqldb* db, const char* buffer, int len);
typedef int (*cubesql_send_enddata_t)(csqldb* db);
typedef char* (*cubesql_receive_data_t)(csqldb* db, int* len, int* is_end_chunk);

typedef int64 (*cubesql_affected_rows_t)(csqldb* db);
typedef int64 (*cubesql_last_inserted_rowID_t)(csqldb* db);

// API declarations
class SQLAPI_API cubeSqlAPI : public IsaAPI
{
public:
	cubeSqlAPI();
...
public:
	...
	cubesql_send_data_t cubesql_send_data;
	cubesql_send_enddata_t cubesql_send_enddata;
	cubesql_receive_data_t cubesql_receive_data;

	cubesql_affected_rows_t cubesql_affected_rows;
	cubesql_last_inserted_rowID_t cubesql_last_inserted_rowID;

	void SetTraceFunction(SAConnection& con, cubesql_trace_callback trace, void *arg);
...
public:
	void Check(csqldb* pDb, const SAString& sCommandText, int nError);
};

...
typedef bool (STDCALL *mysql_eof_t)(MYSQL_RES *res);
...
typedef bool (STDCALL *mysql_ssl_set_t)(MYSQL *mysql, const char *key,
				      const char *cert, const char *ca,
				      const char *capath, const char *cipher);

typedef MYSQL *		(STDCALL *mysql_connect_t)(MYSQL *mysql, const char *host,
				      const char *user, const char *passwd);
typedef bool		(STDCALL *mysql_change_user_t)(MYSQL *mysql, const char *user, 
...
typedef bool	(STDCALL *mysql_read_query_result_t)(MYSQL *mysql);
...
typedef bool (STDCALL *mysql_thread_init_t)(void);
...
typedef bool (STDCALL *mysql_stmt_attr_set_t)(MYSQL_STMT *stmt,
                                    enum enum_stmt_attr_type attr_type,
                                    const void *attr);
typedef bool (STDCALL *mysql_stmt_attr_get_t)(MYSQL_STMT *stmt,
                                    enum enum_stmt_attr_type attr_type,
                                    void *attr);
typedef bool (STDCALL *mysql_stmt_bind_param_t)(MYSQL_STMT * stmt, MYSQL_BIND * bnd);
typedef bool (STDCALL *mysql_stmt_bind_result_t)(MYSQL_STMT * stmt, MYSQL_BIND * bnd);
typedef bool (STDCALL *mysql_stmt_close_t)(MYSQL_STMT * stmt);
typedef bool (STDCALL *mysql_stmt_reset_t)(MYSQL_STMT * stmt);
typedef bool (STDCALL *mysql_stmt_free_result_t)(MYSQL_STMT *stmt);
typedef bool (STDCALL *mysql_stmt_send_long_data_t)(MYSQL_STMT *stmt, 
...
// Special API for detect MariaDb version
enum sa_mariadb_value {
	MARIADB_CHARSET_ID,
...
typedef bool (STDCALL *mariadb_get_info_t)(MYSQL* mysql, enum sa_mariadb_value value, void* arg);

// API declarations
class SQLAPI_API myAPI : public IsaAPI
{
public:
	myAPI();
...
public:
	...
	mysql_get_client_version_t mysql_get_client_version;
	...
	mariadb_get_info_t mariadb_get_info;
...
public:
	...
	void CancelCommand(const SACommand* pCmd, MYSQL* mysql);
};
...
public:
	...
	MYSQL* mysqlKill; // MySQL connection struct for killing the main connection thread
	bool connected;
};
...
#define SACON_OPTION_MYSQL_UTF8_CHARSET_NAME _TSA("Utf8CharacterSetName")

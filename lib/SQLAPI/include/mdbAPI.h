...
typedef my_bool (STDCALL *mariadb_get_info_t)(MYSQL* mysql, enum mariadb_value value, void* arg);

// API declarations
class SQLAPI_API mdbAPI : public IsaAPI
{
public:
	mdbAPI();
...
public:
	...
	// MariaDB statement API functions
...
class SQLAPI_API mdbConnectionHandles : public saConnectionHandles
{
public:
	mdbConnectionHandles();

	MYSQL *mysql;	// MariaDB connection struct
	MYSQL* mysqlKill; // MariaDB connection struct for killing the main connection thread
	bool connected;
};

class SQLAPI_API mdbCommandHandles : public saCommandHandles
{
public:
	mdbCommandHandles();

	MYSQL_RES *result; // MariaDB result struct
	MYSQL_STMT *stmt; // MariaDB statement struct
};

#define SACON_OPTION_MARIADB_CHARSET _TSA("CharacterSet")
#define SACON_OPTION_MARIADB_UTF8_CHARSET_NAME _TSA("Utf8CharacterSetName")

#define SACMD_OPTION_MARIADB_RESULT _TSA("HandleResult")
#define SACMD_OPTION_MARIADB_IGNORE_BINARY_FLAG _TSA("IgnoreFieldBinaryFlag")

#endif // !defined(__MDBAPI_H__)

...
// API declarations
class SQLAPI_API sybAPI : public IsaAPI
{
public:
	sybAPI();
...
protected:
	...
	void UnInitEnv();
	CS_INT GetClientLibraryVersion(const SAOptions *pOptions);
...
public:
	...
	// last API pointer used for the error checking
	static sybAPI* pLastAPI;

	static SASybErrInfo* getSybErrInfo(const sybAPI* pAPI, CS_CONTEXT* context, CS_CONNECTION* connection);
	static CS_RETCODE CS_PUBLIC DefaultClientMsg_cb(CS_CONTEXT* context, CS_CONNECTION* connection, CS_CLIENTMSG* message);
	static CS_RETCODE CS_PUBLIC DefaultServerMsg_cb(CS_CONTEXT* context, CS_CONNECTION* connection, CS_SERVERMSG* message);
};
